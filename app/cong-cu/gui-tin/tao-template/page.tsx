...
// ── Preview panel (Step 2 right panel) ───────────────────────────────────────

function Step2RightPanel({ dark, setDark, title, blocks, actionButtons, templateType, logoMode, uploadedImages }: {
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {["Giao diện sáng", "Giao diện tối"].map((label, i) => (
                      <div key={i}>
                        ...
                        <div className={cn("h-24 rounded border-2 border-dashed flex items-center justify-center cursor-pointer hover:border-blue-400 transition-colors",
                          i === 1 ? "bg-gray-900 border-gray-600" : "bg-white border-gray-300")}>
        ...
        <section className="mb-6">
          ...
          {btnOpen && (
            <div className="space-y-3">
              ...
              {actionButtons.length === 0 && (
                <p className="text-xs text-muted-foreground text-center py-2">Chưa có nút nào — nhấn để thêm</p>
              )}
            </div>
          )}
        </section>
...
function Step3({
  title, blocks, note, setNote, agreed, setAgreed, dark, setDark, templateType, actionButtons,
}: {
  ...
  templateType: string; actionButtons: ActionButton[]
}) {
    ...
    <div className="fixed top-[36px] inset-x-0 bottom-0 z-[90] bg-white flex flex-col">
      ...
      <div className="flex items-center justify-between px-8 py-4 border-t border-border bg-white shrink-0">
        ...
        {step < 2
          ...
          : <Button onClick={() => setDone(true)} disabled={!agreed} className="bg-blue-600 hover:bg-blue-700 text-white px-8">Gửi duyệt</Button>
        }
      </div>
    </div>
  )
