...
const MOCK_BUDGETS: Budget[] = [
  { id: 1,  name: "ZNS Q2/2026", oas: mkOas([["TP",0],["KN",1],["MH",2],["VT",3],["BD",4]]), fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent:  67_500_000, status: "binh-thuong" },
  { id: 2,  name: "ZNS Q2/2026", oas: mkOas([["TP",0],["KN",1]]),                             fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent: 246_000_000, status: "vuot-muc"    },
  { id: 3,  name: "ZNS Q2/2026", oas: mkOas([["MH",2],["VT",3],["LG",5]]),                    fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent:  67_500_000, status: "da-ket-thuc" },
  { id: 4,  name: "ZNS Q2/2026", oas: mkOas([["TP",0],["MH",2],["VT",3],["BD",4]]),           fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent: 180_000_000, status: "canh-bao"    },
  { id: 5,  name: "ZNS Q2/2026", oas: mkOas([["KN",1],["VT",3]]),                             fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent: 170_000_000, status: "canh-bao"    },
  { id: 6,  name: "ZNS Q2/2026", oas: mkOas([["TP",0],["KN",1],["MH",2],["VT",3]]),           fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent:  67_500_000, status: "binh-thuong" },
  { id: 7,  name: "ZNS Q2/2026", oas: mkOas([["MH",2],["BD",4],["LG",5]]),                    fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent:  67_500_000, status: "binh-thuong" },
  { id: 8,  name: "ZNS Q2/2026", oas: mkOas([["TP",0],["KN",1],["MH",2],["VT",3],["BD",4]]), fromDate: "01/04/2026", toDate: "30/06/2026", limit: 200_000_000, spent:  67_500_000, status: "binh-thuong" },
  { id: 9,  name: "ZNS Q1/2026", oas: mkOas([["VT",3],["BD",4]]),                             fromDate: "01/01/2026", toDate: "31/03/2026", limit: 150_000_000, spent:  60_000_000, status: "da-ket-thuc" },
  { id: 10, name: "ZNS Q1/2026", oas: mkOas([["TP",0],["KN",1],["MH",2]]),                    fromDate: "01/01/2026", toDate: "31/03/2026", limit: 150_000_000, spent:  55_000_000, status: "binh-thuong" },
]
...
function ProgressBar({ limit, spent, status }: { limit: number; spent: number; status: BudgetStatus }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={cn("text-xs font-semibold tabular-nums w-10 text-right", status === "vuot-muc" ? "text-red-600" : status === "canh-bao" ? "text-orange-600" : "text-gray-600")}>
        {pct}%
      </span>
    </div>
  )
...
function OaAvatarStack({ oas }: { oas: { initials: string; color: string }[] }) {
  ...
  return (
    <div className="flex items-center mt-1">
      {show.map((oa, i) => (
        <span key={i} className="inline-flex h-5 w-5 rounded-full items-center justify-center text-[8px] font-bold text-white border border-white -ml-1 first:ml-0" style={{ background: oa.color }}>
          {oa.initials.slice(0, 1)}
        </span>
      ))}
      {rest > 0 && (
...
function CreateModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: (name: string) => void }) {
  // Section 1
  ...
  function toggleOa(id: string) {
    setSelOas(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-2xl w-[440px] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        {/* Header */}
        ...
        <div className="px-5 py-4 space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={budgetName} onChange={e => setBudgetName(e.target.value)}
                       placeholder="Nhập tên ngân sách" className="w-full px-3 py-2 text-sm border border-border rounded-lg outline-none focus:border-blue-400 placeholder:text-muted-foreground" />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input value={limit} onChange={e => setLimit(e.target.value.replace(/\D/g, ""))}
                         placeholder="Nhập hạn mức, tối thiểu 50.000" className="w-full px-3 py-2 pr-7 text-sm border border-border rounded-lg outline-none focus:border-blue-400 placeholder:text-muted-foreground" />
          ...
          <SectionBox num={3} title="ĐẶT NGƯỠNG CẢNH BÁO" hasHelp>
            <div className="space-y-2">
                ...
                <div key={th.id} className="flex items-center gap-2 text-sm">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <input
                      ...
                      className={cn("w-12 text-center border border-border rounded px-1 py-0.5 text-xs outline-none", th.system ? "bg-gray-50 text-muted-foreground" : "focus:border-blue-400")}
                    />
              ...
              <button
                onClick={() => setThresholds(prev => [...prev, { id: nextThId(), pct: "" }])}
              ...
              {notifOn && (
                <>
                  ...
                  <div className="flex flex-wrap gap-1.5 p-2 border border-border rounded-lg min-h-[36px]">
                    ...
                    <div className="relative">
                      <button className="flex items-center text-muted-foreground hover:text-foreground px-1">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {ROLES.filter(r => !selRoles.includes(r)).map(r => (
                      <button key={r} onClick={() => toggleRole(r)} className="text-xs border border-dashed border-border rounded px-2 py-0.5 text-muted-foreground hover:border-blue-400 hover:text-blue-600 transition-colors">
                        + {r}
                      </button>
                    ))}
                  </div>
...
function Checkbox({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={cn("h-4 w-4 rounded border flex items-center justify-center shrink-0 transition-colors", checked ? "border-blue-500 bg-blue-500" : "border-gray-300 bg-white")}
    >
    ...
    <SidebarProvider>
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-5 h-[calc(100vh-56px)]">
          ...
          {isEmpty ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center space-y-4">
              <img src="https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/svg/1f4b0.svg" className="h-20 w-20 opacity-80" alt="" />
              ...
              {isNullSearch ? (
                ...
                <div className="rounded-lg border border-border bg-white overflow-hidden">
                  ...
                  <div className="grid text-xs font-semibold text-muted-foreground bg-gray-50 border-b border-border px-4 py-2.5"
                    ...
                    <span>Thời hạn</span>
                    <span className="flex items-center gap-0.5 cursor-pointer hover:text-foreground">Ngân sách (VND) <ChevronDown className="h-3 w-3" /></span>
                    <span className="flex items-center gap-0.5 cursor-pointer hover:text-foreground">Đã tiêu (VND) <ChevronDown className="h-3 w-3" /></span>
                  ...
                  {paged.map((b, i) => (
                    <div key={b.id}
                         className={cn("grid items-center px-4 py-3 hover:bg-gray-50 transition-colors cursor-pointer", i < paged.length - 1 ? "border-b border-border" : "")}
                         style={{ gridTemplateColumns: "2fr 1.5fr 1.5fr 1.5fr 2fr 1.2fr" }}>
                      <div>
                      ...
                      <span className="text-sm font-medium">{fmtVnd(b.limit)}</span>
                      <span className="text-sm">{fmtVnd(b.spent)}</span>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {Array.from({ length: Math.min(totalPages, 6) }, (_, i) => i + 1).map(p => (
                      <button key={p} onClick={() => setPage(p)}
                              className={cn("h-7 w-7 rounded border text-xs flex items-center justify-center", page === p ? "border-blue-500 text-blue-600 font-semibold bg-blue-50" : "border-border hover:bg-gray-50")}>
                        {p}
                      </button>
                    ))}
                    ...
                    {totalPages > 6 && (
                      <button onClick={() => setPage(totalPages)}
                              className={cn("h-7 w-7 rounded border text-xs flex items-center justify-center", page === totalPages ? "border-blue-500 text-blue-600 font-semibold bg-blue-50" : "border-border hover:bg-gray-50")}>
                        {totalPages}
                      </button>
                    )}
      ...
      {successName && (
        <SuccessModal
          ...
          onViewDetail={() => setSuccessName(null)}
        />
      )}
    </SidebarProvider>
  )
